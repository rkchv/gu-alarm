...
//---------------------------------------------------------------------------
	...
	// версия библиотеки
	#define 	VERMAJOR_RTUSBAPI 				(1)    	// только одна цифра
	#define 	VERMINOR_RTUSBAPI 				(4)		// только одна цифра
	#define 	CURRENT_VERSION_RTUSBAPI     	((VERMAJOR_RTUSBAPI<<16)|VERMINOR_RTUSBAPI)
	...
	extern "C"  LPVOID WINAPI RtCreateInstance( const PCHAR DeviceName );
	...
	struct IRTUSB
	{
		...
		virtual BOOL WINAPI ReleaseDevice(void) = 0;
	};
	...
	namespace RTUSB3000
	{
		...
		enum {	NO_SYNCHRO, TTL_START_SYNCHRO, TTL_KADR_SYNCHRO, 
		         ANALOG_SYNCHRO, INVALID_INPUT_SYNCHRO };
